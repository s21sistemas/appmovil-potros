...
const handleSubmit = async () => {
  ...
  // Procesar pagos (separado para mejor organización)
    ...
    try {
        // Determinar la colección de costos según el tipo de inscripción
        ...
        // Filtrar costos por temporada activa
        const costosFiltrados = costosSnapshot.docs.filter(doc => {
            const data = doc.data();
            return data.temporadaId && data.temporadaId.value === temporadaActiva.value;
        });

        if (costosFiltrados.length === 0) {
            throw new Error(`No se encontraron costos configurados para la temporada ${temporadaActiva.label}`);
        }

        // Para jugadores, también filtramos por categoría si es necesario
        let costoRelevante;
        if (formData.tipo_inscripcion === 'porrista') {
            costoRelevante = costosFiltrados[0].data(); // Porristas no tienen categoría
        } else {
            // Buscar costo que coincida con categoría del jugador
            const costoPorCategoria = costosFiltrados.find(doc => {
                const data = doc.data();
                return data.categoria === formData.categoria;
            });
            
            if (!costoPorCategoria) {
                throw new Error(`No se encontraron costos configurados para la categoría ${formData.categoria}`);
            }
            costoRelevante = costoPorCategoria.data();
        }
        ...
        if (formData.tipo_inscripcion === 'porrista') {
            const inscripcion = parseCost(costoRelevante.inscripcion);
            const coaching = parseCost(costoRelevante.coaching);
            const total = inscripcion + coaching;
/*
            const pagosPorrista1 = {
                porristaId: playerId,
          ...
          const pagosPorrista = {
            ...
            const inscripcion = parseCost(costoRelevante.inscripcion);
            const coaching = parseCost(costoRelevante.coaching);
            const tunel = parseCost(costoRelevante.tunel);
            const botiquin = parseCost(costoRelevante.botiquin || '0'); // Por si no existe en algunos casos
            const total = inscripcion + coaching + tunel + (botiquin || 0);
/*
            const pagosJugador1 = {
                jugadorId: playerId,
                nombre: nombreCompleto,
  ...
  return (
      ...
      <View style={styles.formContainer}>
        ...
        <View style={styles.regulationContainer}>
          ...
          <TouchableOpacity 
            onPress={() => Linking.openURL('https://firebasestorage.googleapis.com/v0/b/potros-632ee.firebasestorage.app/o/reglamento%2FReglamentoPotros.pdf?alt=media&token=35addb4c-86f6-404d-aa4d-5c156ca1d307')} 
